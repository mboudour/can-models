...
The selected Pew source is Wave 152, fielded 12–18 August 2024. The workspace links the official dataset page, report, methodology, final questionnaire, and topline; shows a theory-led 15-node mapping; and logs every planned and non-executed CAN component. It deliberately excludes conditional chatbot-use questions and form-specific batteries from the first mapping, preserving an all-respondent estimand.

The official microdata download currently requires a free, email-verified Pew Research Center account and agreement to Pew's Terms of Use. The app therefore does not attempt to bypass the access flow, request an account email, bundle raw data, or show a fabricated network. It instead supplies a downloadable source manifest and requires file-level verification, documented nonresponse recoding, response-distribution inspection, complete-case reporting, and a non-saturation/robustness gate before any public results can appear. The full scope is in [pew_atp_w152_ai_case.md](pew_atp_w152_ai_case.md).
